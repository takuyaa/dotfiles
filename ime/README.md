...
Karabiner のタップ判定は 200ms で、`kanata.kbd` の tap-hold タイムアウトと揃えて
あります。ホールドが出にくい／タップが出にくい場合は
`karabiner-ime-switch.json` の `basic.to_if_alone_timeout_milliseconds` を調整
してください。
