...
| キー | タップ | ホールド |
|-----|--------|----------|
| 無変換（スペース左） | F13（→ 英数 / IME OFF） | SHORTCUTS レイヤー: `a/c/f/v/x/y/z/s` → Ctrl+A/C/F/V/X/Y/Z/S |
| 変換（スペース右）   | F14（→ かな / IME ON）  | NUMS レイヤー: `q w e … p` → `1 2 … 0`、`1 2 … 0` → `F1 F2 … F10` |
...
無変換は親指ホームポジションなので、Win キーより届きやすく常用に向きます。SHORTCUTS レイヤーから
**右 Ctrl** で発火するため左 Ctrl ベースの Emacs 割り当てに横取りされません。
...
注意: ホールド判定は「もう片方のキーを離した瞬間」なので、`無変換↓ → c↓ → c↑ → 無変換↑`
の順なら IME は切り替わりません。逆に `無変換↓ → c↓ → 無変換↑ → c↑` のように先に
無変換を離した場合だけ tap 判定で F13 が漏れ、IME が「英数（直接入力）」に切り替わります
（仕様として許容）。Win キーは素のままなので、Win+L / Win+D / Win+E / Win+1〜9 などの
Windows 標準コンボはそのまま使えます。
