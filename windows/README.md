...
- **入力切替ホットキーの無効化:** `HKCU\Keyboard Layout\Toggle` の
  `Language Hotkey` / `Layout Hotkey` / `Hotkey` を `3`（無効）にし、
  `Alt+Shift` / `Ctrl+Shift` での勝手な IME 切替を止めます。日本語言語は土台の
  キーボードレイアウト（`00000411`、置換で「日本語 (US)」表示）をサインイン時に
  必ず復活させるため、この項目の削除ではなく切替の封じ込めで対処します。
  残る `Win+Space` の誤爆は、設定 → 入力 → キーボードの詳細設定で「アプリ
  ウィンドウごとに異なる入力方式を設定する」をオフ＋「既定の入力方式の上書き」を
  Google日本語入力にして塞ぎます（ユーザー単位の UI 状態のため DSC 外）。
  **サインアウト → サインインが必要。**
- **既定の入力方式を Google日本語入力に固定:** `HKCU\Control Panel\International\User Profile`
  の `InputMethodOverride` に Google IME の TIP（`0411:{...}{...}`）を設定します
  （既定の入力方式の上書き / `Set-WinDefaultInputMethodOverride` 相当）。サインイン時などの
  既定の入力方式が、日本語言語の素のキーボードレイアウトではなく Google日本語入力になります。
  日本語言語自体は設定アプリで追加済みである前提（この値は既定の指定のみ）。
  **サインアウト → サインインが必要。**
...
| キー | タップ | ホールド |
|-----|--------|----------|
| 無変換（スペース左） | F13（→ 英数 / IME OFF） | **右 Ctrl**（押している間、任意キー = Ctrl+任意キー。Tab だけ super-alt-tab） |
...
単に **右 Ctrl** を押した状態にするので、列挙なしで `無変換+a〜z`、`無変換+1〜9`（タブ切替）
など Ctrl 系すべてが効きます（`c` `v` `x` `z` `s` `g` など defsrc に無いキーも含む）。Tab
だけは例外で、Alt を保持したままウィンドウを切り替える **super-alt-tab**（下記）になります。
...
`on-release` で Alt を離して確定します（保険として ~1.5 秒アイドルでも解放）。無変換ホールドは
右 Ctrl を握ったままなので、そのまま Tab を送ると `Ctrl+Alt+Tab`（Windows の別モードの固定
スイッチャ）になってしまいます。これを防ぐため `@atab` は `(unmod (rctl lctl) tab)` で、Tab を
送る間だけ握っている Ctrl を一時解除します（Shift は解除しないので `無変換+Shift+Tab` の
逆順は維持）。この方式なら Ctrl を出しっぱなしにできるので、`無変換+c` などの Ctrl 系は
すべて従来どおり効きます。

代償: `無変換+Tab` から `Ctrl+Tab`（ブラウザのタブ切替）は出なくなります。物理 Ctrl で取ります。
